...
class Donation(models.Model):
    ...
    collectionloc = models.CharField(max_length=300, null=True, verbose_name="Collection Location")
    ...
    def __str__(self):
        return f"Donation {self.id} - {self.get_donationname_display()}"
    
class Gallery(models.Model):
    donation = models.ForeignKey(Donation, on_delete=models.CASCADE)
    deliverypic = models.ImageField(upload_to='delivery_pics/', null=True, blank=True)  # set upload path
    creationdate = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Gallery Item {self.id} for Donation {self.donation.id}"
